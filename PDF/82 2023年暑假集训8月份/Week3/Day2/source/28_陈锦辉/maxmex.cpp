...
int n,q;
int a[MAXN];
void add(int x,int *c,bitset<MAXN>&b){if(!c[x]++)b[x]=0;}
void del(int x,int *c,bitset<MAXN>&b){if(!--c[x])b[x]=1;}
void solve_subtask1(){
	static int c[MAXN];
	...
	for(int k;q--;){
		...
		int ans=0;
		for(int i=1;i<=k;i++) add(a[i],c,b);
		ans=max(ans,(int)b._Find_first());
		for(int i=k+1;i<=n;i++){
			add(a[i],c,b),del(a[i-k],c,b);
			ans=max(ans,(int)b._Find_first());
		}
		printf("%d\n",ans);
	}
