...
void dfs(const int u,const int t,const int sum){
	if(u==t) Max(ans,sum);
	for(const PII&V:G[u]){
		int v=V.first,w=V.second;
		if(1ll*sum*w<=m) dfs(v,t,sum*w);
	}
}
int main(){
	...
	for(int w=2,u,v;w<=m;w++)std::scanf("%d%d",&u,&v),G[u].emplace_back(v,w),G[v].emplace_back(u,w);
	for(int s,t;q--;){
		std::scanf("%d%d",&s,&t);
		ans=-1;
		dfs(s,t,1);
