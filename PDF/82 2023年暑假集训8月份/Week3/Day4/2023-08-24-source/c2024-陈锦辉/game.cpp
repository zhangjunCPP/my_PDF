...
void solve_st3(){
	...
	do{
		...
		LL w=0;
		for(int i=1;i<=n;i++){
			for(;!s.empty() && s.top()>=p[i];s.pop()) w+=a[p[i]];
			s.push(p[i]);
		}
		if(w>ans) ans=w,memcpy(q,p,sizeof p);
...
int main(){
	...
	scanf("%d",&n);
	bool st1=1,st2=1;
	for(int i=1;i<=n;i++) scanf("%d",&a[i]),st1&=a[i]==i,st2&=a[i]==n-i+1,p[i]=i;
	if(st1) solve_st1();
	else if(st2) solve_st2();
	else if(n<10) solve_st3();
	else solve_st4();
	return 0;
}
/*
