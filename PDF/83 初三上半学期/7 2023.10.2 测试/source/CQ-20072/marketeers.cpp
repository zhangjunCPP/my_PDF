...
int Ch,n,i,cnt[N],j,k,k1,tot,L,R,s,F[2][N],*f,*lf,G_cnt[N],*p_tmp,mi[N];
ll ans=1;
...
int main(){
	...
	*Begin=a[0].begin();
	...
	for(i=1;i<=n;i++){
		it2=mp.lower_bound(*Begin[i]);
		if(it2==mp.end()||mi[it2->second]>a[i].back())it2=mp.emplace(a[i].back(),++tot).first;
		Group[it2->second].emplace_back(i);
		mi[it2->second]=*Begin[i];
		G_cnt[it2->second]++;
	}
	for(i=1;i<=tot;i++){
		f=*F;
		...
		for(j=G_cnt[i]-1;~j;j--){
			...
			s=R=0;
			for(L=0;L<cnt[k];L++){
				while(R<cnt[k1]&&Begin[k][L]>Begin[k1][R]){
					s+=lf[R++];
					if(s>=M)s-=M;
				}
				f[L]=s;
