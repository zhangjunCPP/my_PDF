...
signed main(){
	for(int i=1;i<=1000000000;i++){
		puts("making data..."); 
		system("rd.exe < ans.txt > rd.txt");
		puts("make data successfully"); 
		int x=clock();
		system("ans.exe < rd.txt > ans.txt");
		cout<<"ans.time:"<<clock()-x<<endl;
		system("bl.exe < rd.txt > bl.txt");
		if(system("fc ans.txt bl.txt")){
			system("pause");
			system("pause");
			system("pause");
			return 0;
