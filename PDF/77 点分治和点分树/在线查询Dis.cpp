...
inline int Dis( Re x, Re y )
{
	return(deep[x] + deep[y] - (deep[T1.lca( x, y )] << 1) );
}   /* 查询原树中点x,y的距离 */
...
inline void sakura( Re x, Re Fa )                 /* 处理重心x所囊括的连通块 */
{
	...
	TR1[x].build( now / 2 + 1 ), TR2[x].build( now + 1 );  
    /* 由重心性质可知,TR1会使用[0,now/2],TR2会使用[1,now]，
    向后移一位变为[1,now/2+1]和[2,now+1] */
	for ( Re i = head[x], to; i; i = a[i].next )
		if ( !vis[to = a[i].to] )
			sum = size[to] > size[x] ? 
            	now - size[x] : 
    			size[to], maxp[rt = 0] = inf, getrt( to, 0 ), sakura( rt, x );
	            /* 注意子连通块大小不要直接用size[to] */
}
